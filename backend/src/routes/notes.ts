...
import { validateCreateNote, validateUpdateNote, validateId, validateUserId } from '../middleware/validation';
...
router.get('/', asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
  ...
  if (search) {
    notes = await noteRepository.search(search as string, userId as string);
...
router.get('/user/:userId', validateUserId, asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
...
router.post('/', validateCreateNote, asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
...
router.put('/:id', validateUpdateNote, asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
  const { id } = req.params;
  ...
  const note = await noteRepository.update({ id, ...updateData } as UpdateNoteInput);
  res.json({
    success: true,
    data: note,
