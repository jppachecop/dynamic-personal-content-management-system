...
router.post('/', validateCreateUser, asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
...
router.put('/:id', validateUpdateUser, asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
  const { id } = req.params;
  ...
  const user = await userRepository.update({ id, ...updateData } as UpdateUserInput);
  res.json({
    success: true,
    data: user,
